...
router.post("/post-result", async (req, res) => {
  try {
    ...
    const missingFields = [];
    if (!result_user_id) missingFields.push("result_user_id");
    if (!result_test_id) missingFields.push("result_test_id");
    if (result_score == null) missingFields.push("result_score"); // important: score=0 is valid
    if (result_total_score == null) missingFields.push("result_total_score");
    if (!result_poc_id) missingFields.push("result_poc_id");
    ...
    if (existingResult) {
      return res.status(409).json({
        message: "Result already exists for this user and test"
      });
    }
    ...
    console.error("Error saving result:", error); // Log full error
