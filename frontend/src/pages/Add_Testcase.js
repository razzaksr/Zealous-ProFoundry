...
const Add_Testcase = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/testcase/create_testCase', {
        method: 'POST',
    ...
    <Container maxWidth="md" sx={{ minHeight: "90vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
      <Paper elevation={6} sx={{ width: "100%", overflow: "hidden", borderRadius: "16px" }}>
        ...
        <Box sx={{ background: "linear-gradient(90deg, #3f51b5 0%, #5c6bc0 100%)", padding: "20px 24px", color: "white" }}>
          <Typography variant="h5" fontWeight="600">Create Test Case</Typography>
          <Typography variant="subtitle2" sx={{ opacity: 0.8, mt: 0.5 }}>Design a new Test Case for your quiz or assessment</Typography>
        </Box>

        {/* Form Content */}
        <Box sx={{ padding: "24px" }}>
          {/* Input Field */}
          <Typography variant="subtitle1" fontWeight="500" sx={{ mb: 1 }}>Testcase Input</Typography>
          <TextField
            fullWidth
          ...
          {/* Output Field */}
          <Typography variant="subtitle1" fontWeight="500" sx={{ mb: 1 }}>Testcase Output</Typography>
          <TextField
            fullWidth
      ...
      <Snackbar open={openError} autoHideDuration={3000} onClose={handleCloseSnackbar} anchorOrigin={{ vertical: "top", horizontal: "right" }}>
        <Alert onClose={handleCloseSnackbar} severity="error" sx={{ width: "100%" }} icon={<ErrorOutlineIcon />}>
          Please fill both input and output fields.
        </Alert>
      </Snackbar>
    </Container>
    </>
