...
const handleFinalSubmit = async (isMalpractice = false) => {
  ...
  try {
    // Fetch test result data from localStorage
    const getStoredTestResult = () => {
      try {
        const storedTestResult = localStorage.getItem('test_result');
        if (storedTestResult) {
          return JSON.parse(storedTestResult);
        }
      } catch (error) {
    ...
    const storedResult = getStoredTestResult();

    const resultData = {
      result_user_id: storedResult.result_user_id || testResult.result_user_id || userId || "",
      result_test_id: storedResult.result_test_id || testResult.result_test_id || testId || "",
      result_poc_id: storedResult.result_poc_id || testResult.result_poc_id || pocId || "",
      result_score: storedResult.result_score || testResult.result_score || 0,
      result_total_score: storedResult.result_total_score || testResult.result_total_score || 0,
      codingAnswered: storedResult.codingAnswered || testResult.codingAnswered || 0,
      codingCorrect: storedResult.codingCorrect || testResult.codingCorrect || 0,
      codingIds: storedResult.codingIds || testResult.codingIds || [],
      codingNotAnswered: storedResult.codingNotAnswered || testResult.codingNotAnswered || 0,
      codingNotVisited: storedResult.codingNotVisited || testResult.codingNotVisited || 0,
      codingWrong: storedResult.codingWrong || testResult.codingWrong || 0,
      marked: storedResult.marked || testResult.marked || 0,
      mcqAnswered: storedResult.mcqAnswered || testResult.mcqAnswered || 0,
      mcqCorrect: storedResult.mcqCorrect || testResult.mcqCorrect || 0,
      mcqNotAnswered: storedResult.mcqNotAnswered || testResult.mcqNotAnswered || 0,
      mcqNotVisited: storedResult.mcqNotVisited || testResult.mcqNotVisited || 0,
      mcqWrong: storedResult.mcqWrong || testResult.mcqWrong || 0,
      studentName: storedResult.studentName || testResult.studentName || studentName || "",
      testLanguage: storedResult.testLanguage || testResult.testLanguage || "",
      testName: storedResult.testName || testResult.testName || "",
      codingResults: storedResult.codingResults || testResult.codingResults || [],
    };
