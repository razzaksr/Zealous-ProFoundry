...
import { fetchStudents, sendStudentRankings } from "../axios"; // Import API functions
...
const ClassPerformance = () => {
  ...
  const handleSendStudentRankings = async (pocId) => {
    try {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      // Trigger the DailyPerformanceCharts PDF export
      const chartsButton = document.querySelector("#daily-performance-charts button");
      if (chartsButton) {
        chartsButton.click();
      } else {
        console.warn("Daily Performance Charts button not found");
      }
    } catch (err) {
          ...
          >
            ...
            {allFiltersSelected ? (
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const pocId = selectedPocData
                            ...
                            : null;
                          if (pocId) {
                            handleSendStudentRankings(pocId);
                          } else {
                  ...
                  >
                    <CardContent>
                      ...
                      <Typography
                        ...
                        sx={{ mb: 3 }}
                      >
                        Detailed performance for each test date
                      </Typography>
                      <Divider sx={{ mb: 3 }} />
                      {filteredStudents.length === 0 ? (
                        ...
                        [...new Set(
                          ...
                          .sort()
                          .map((date, dateIndex) => {
                            const dailyData = filteredStudents
                              .map((student) => {
              ...
              <Grow in={loadedData} timeout={1400}>
                ...
                >
                  <Typography variant="subtitle1" fontWeight="medium">
                    Please select a college, module, and POC to view detailed
                    performance data
                  </Typography>
                  <Typography variant="body2" sx={{ mt: 1 }}>
